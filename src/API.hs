...
apiDef = [ (DData "" (MkDData "API" (DPi (Nothing) DType (DPi (Nothing) DType DType)) [ (MkDTy "Api" "" (DPi (Just "input") DType (DPi (Just "output") DType (DApp (DApp (DRef "API") (DRef "input")) (DRef "output"))))) ]))
         ...
         , (DMutual [ (DData "" (MkDData "Id" (DPi (Just "ty") DType (DPi (Nothing) DType DType)) [ (MkDTy "Id'" "" (DPi (Nothing) (DRef "rep") (DApp (DRef "Id") (DRef "rep")))) ]))
                    , (DMutual [ (DRecord "" "Date" [  ] Nothing [ (MkDField "" "year" (DPrimVal CIntType))
                               ...
                               , (DDef [ (MkDPatClause (DRef "UserId") (DApp (DTermNotSupported "NamedApp") (DPrimVal CStringType))) ]) ]) ])
