...
instance Pretty DDecl where
  pretty (DData doc (MkDData name term tyds)) = text name $$ (nest 4 (vcat $ map pretty tyds))
  pretty (DRecord doc name con fs) = text name $$ (nest 4 (vcat $ map pretty fs))
  pretty _ = empty
...
instance Pretty DTypeDecl where
  pretty (MkDTy n doc typ) = text n $$ (nest 4 $ colon <+> pretty typ)

instance Pretty DField where
  pretty (MkDField doc n typ) = text n <+> colon <+> pretty typ
